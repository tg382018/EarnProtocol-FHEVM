...
contract EarnProtocol is SepoliaConfig {
    ...
    function calculateEncryptedScore(
        ...
        // Calculate encrypted score using weighted formula
        euint64 score = FHE.asEuint64(0);
        
        // Wallet age (max 200 points) - 1 year = 100 points
        // Formula: min(200, walletAge * 0.27)
        euint64 walletAgeScore = FHE.mul(encryptedWalletAge, FHE.asEuint64(27));
        walletAgeScore = FHE.div(walletAgeScore, FHE.asEuint64(100));
        if (FHE.gt(walletAgeScore, FHE.asEuint64(200))) {
            walletAgeScore = FHE.asEuint64(200);
        }
        score = FHE.add(score, walletAgeScore);
        
        // Transaction count (max 300 points) - 100 tx = 100 points
        // Formula: min(300, transactionCount * 1.0)
        euint64 txCountScore = encryptedTransactionCount;
        if (FHE.gt(txCountScore, FHE.asEuint64(300))) {
            txCountScore = FHE.asEuint64(300);
        }
        score = FHE.add(score, txCountScore);
        
        // ETH balance (max 200 points) - 1 ETH = 100 points
        // Formula: min(200, ethBalance * 100)
        euint64 ethBalanceScore = FHE.mul(encryptedEthBalance, FHE.asEuint64(100));
        if (FHE.gt(ethBalanceScore, FHE.asEuint64(200))) {
            ethBalanceScore = FHE.asEuint64(200);
        }
        score = FHE.add(score, ethBalanceScore);
        
        // Gas usage (max 150 points) - 1M gas = 50 points
        // Formula: min(150, totalGasUsed / 20000)
        euint64 gasScore = FHE.div(encryptedTotalGasUsed, FHE.asEuint64(20000));
        if (FHE.gt(gasScore, FHE.asEuint64(150))) {
            gasScore = FHE.asEuint64(150);
        }
        score = FHE.add(score, gasScore);
        
        // Average transaction value (max 100 points) - 0.1 ETH = 50 points
        // Formula: min(100, averageTxValue * 500)
        euint64 avgTxScore = FHE.mul(encryptedAverageTxValue, FHE.asEuint64(500));
        if (FHE.gt(avgTxScore, FHE.asEuint64(100))) {
            avgTxScore = FHE.asEuint64(100);
        }
        score = FHE.add(score, avgTxScore);
        
        // Unique contracts (max 50 points) - 10 contracts = 25 points
        // Formula: min(50, uniqueContracts * 2.5)
        euint64 contractScore = FHE.mul(encryptedUniqueContracts, FHE.asEuint64(25));
        contractScore = FHE.div(contractScore, FHE.asEuint64(10));
        if (FHE.gt(contractScore, FHE.asEuint64(50))) {
            contractScore = FHE.asEuint64(50);
        }
        score = FHE.add(score, contractScore);
        
        // Cap at 1000
        if (FHE.gt(score, FHE.asEuint64(1000))) {
            score = FHE.asEuint64(1000);
        }
